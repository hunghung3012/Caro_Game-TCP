...
class CaroGame:
    ...
    def make_move(self, row, col):
        if self.my_turn and not self.buttons[row][col]['text']:
            self.buttons[row][col].config(text=self.role)  # Player's move (X or O)
            self.board[row][col] = self.role  # Update logic board
            if self.check_win(row, col, self.role):
                messagebox.showinfo("Game Over", f"{self.role} wins!")
                self.reset_game()
        ...
        while True:
            try:
                ...
                if message.startswith("MOVE"):
                    ...
                    self.buttons[row][col].config(text=opponent_role)  # Opponent's move
                    self.board[row][col] = opponent_role  # Update logic board
                    if self.check_win(row, col, opponent_role):
                        messagebox.showinfo("Game Over", f"{opponent_role} wins!")
                        self.reset_game()
